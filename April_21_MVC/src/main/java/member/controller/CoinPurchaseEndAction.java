		...
		if(super.checkLogin(request) ) {		// 부모에 있는 checkLogin 함수를 실행해서 이것이 true 라면 실행 
			...
			*/
			if(loginuser.getUserid().equals(userid)) {
				// 로그인한 사용자가 자신의 코인을 충전 결제하는 경우 
				
				String coinmoney = request.getParameter("coinmoney");    // cointmoney를 login.jsp 에서 가져옴   // 300000, 200000, 100000 원을 가져온다. radio 에서선택한것을 
				
				
				String productName = "코인충전";     //새우깡
				int productPrice = 100;
				
				request.setAttribute("productName", productName);  // 새우깡을 넘겨주기위해 저장.
				request.setAttribute("productPrice", productPrice);  // 100을 넘겨주기 위해 request에 저장.   String 타입으로 넘겨줄거면 스플릿으로 , 제거 
				
				request.setAttribute("email", loginuser.getEmail());	// 로그인 된 사람의 이메일 저장 
				request.setAttribute("name", loginuser.getName());	    // 로그인 된 사람의 이름 저장 
				request.setAttribute("mobile", loginuser.getMobile());	// 로그인 된 사람의 전화번호 저장 
				
				request.setAttribute("userid", userid);  	// 로그인 된 사람의 userid 저장 	이 사람에 대해서 코인이 올라가여 하므로 중요하다. 
				request.setAttribute("coinmoney", coinmoney); 
				
				
				
				super.setRedirect(false);
				super.setViewPage("/WEB-INF/member/paymentGateway.jsp");			// 이 페이지로 이동시켜 실행 시킨다.
			}
			else {
				// 로그인한 사용자가 다른 사용자의 코인을 충전결제 시도하는 경우 
				String message = "다른 사용자의 코인 충전 결제 시도는 불가합니다.";
