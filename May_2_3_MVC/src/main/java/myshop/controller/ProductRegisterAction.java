	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if( loginuser != null && "admin".equals(loginuser.getUserid())) {
			...
			super.setViewPage("/WEB-INF/myshop/admin/productRegister.jsp");
		       ...
		       */
	            ...
	            */
				...
				String uploadFileDir = svlCtx.getRealPath("/images");   // 진짜 경로 를 뜻함 
				// Dir 은 디렉토리를 뜻함 
				
				//System.out.println("=== 첨부되어지는 이미지 파일이 올라가는 절대 경로 uploadFileDir =>" + uploadFileDir);
				// 결과 : === 첨부되어지는 이미지 파일이 올라가는 절대 경로 uploadFileDir =>C:\NCS\workspace(JSP)\.metadata\.plugins\org.eclipse.wst.server.core\tmp0\wtpwebapps\MyMVC\images
				// .metadata는 운영 경로를 뜻한다.
				// 개발 경로에서 사진을 넣으면 .metadate 운영경로에서 사진이 같이 들어가지만 .metadata 경로 에 직접 사진을 넣으면 개발 경로에서는 전혀 추가되지 않는다.
				// 개발 경로 ==>  C:\NCS\workspace(JSP)\MyMVC\src\main\webapp\images 
	          ...
	          */
				...
				try {
					...
					// 위의 코드 처럼 하면 제품 등록 할때 똑같은 이름이 있다면 뒤에 숫자를 자동으로 붙여서 구분해 준다.
					// 운영 경로에는 사진이 올라가지만 개발 경로에는 사진이 올라가지 않는다.
				}catch(IOException e) {
					request.setAttribute("message", "업로드 되어질 경로가 잘못되었거나 또는 최대 용량 10MB 를 초가했으므로 파일 업로드를 실패하였습니다.");
					request.setAttribute("loc", request.getContextPath()+"/shop/admin/productRegister.up");   // 자기 자신페이지로 절대경로 사용
				...
				// === 첨부 이미지 파일, 제품설명서 파일을 올렸으니 그 다음으로 제품정보 (제품명, 정가, 제품수량, ..... )를 DB 의 tbl_product 테이블에 insert해야 한다 === //
				...
				String pimage1 = mtrequest.getFilesystemName("pimage1");   // 제품 이미지 파일명(파일 서버에 업로드 되어진 실제 파일명)
				String pimage2 = mtrequest.getFilesystemName("pimage2");   // 제품 이미지 파일명(파일 서버에 업로드 되어진 실제 파일명)
				...
				// 확인용 pimage2 => 쉐보레후면1.jpg    중복된것은 뒤에 숫자가 추가되어 나온다.
				
				String prdmanual_systemFileName = mtrequest.getFilesystemName("prdmanualFile");
				// 제품 설명서 파일명(파일 서버에 업로드 되어진 실제 파일명)
				...
				String prdmanual_orginFileName = mtrequest.getOriginalFileName("prdmanualFile"); // () 안은 form 태그 안의 name을 쓴다. (136줄)
				// 웹클라이언트의 웹브라우저에서 파일을 업로드 할때 올리는 제품설명서 파일명
		        // 제품설명서 파일명 입력은 선택사항이므로 NULL 이 될 수 있다.
		        // 첨부파일들 중 이것만 파일다운로드를 해주기 때문에 getOriginalFileName(String name) 메소드를 사용한다. 
				
				
				//System.out.println("~~~확인용 prdmanual_originFileName=>" + prdmanual_orginFileName);
			...
			*/	
				...
				// 시퀀스는 채번해야함
				int pnum = pdao.getPnumOfProduct();// 제품번호 채번해오기 중요한 부분!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~//
				...
				try {
					...
					//매번 숫자가 달라지므로 반복문 사용해야 함  
					for(int i=0; i<attachCount; i++) {
						String attachFileName = mtrequest.getFilesystemName("attach"+i);  // 첨부 File 이름을 알아온다.
