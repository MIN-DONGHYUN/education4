...
-------- **** 장바구니 테이블 생성하기 **** ----------
 desc tbl_member;     -- 회원테이블
 desc tbl_product;    -- 제품 테이블
 ...
 /*
    ...
    ----------------------------------------
      1	       mdh2057	     3	      6	   23/04/25
      2	       mdh2057	     5	      1	   23/04/25
      3	       leess	     3	      2	   23/04/25    
...
select A.cartno, A.fk_userid, A.fk_pnum, B.pname, B.pimage1, B.price, B.saleprice, B.point, A.oqty
from tbl_cart A join tbl_product B
on A.fk_pnum = B.pnum 
where A.fk_userid = 'mdh2057' 
order by A.cartno desc; 


-- 장바구니에 담긴 총 합계를 구해오는 sql 문 
-- 장바구니 없는 사람을 조회해 올때 
...
on B.fk_pnum = C.pnum
where 1=1 
and A.fk_userid = 'mdh2057';



-----------------------------------------------------------------------------------
-- 포인트 임의로 늘리기
update tbl_member set coin = coin + 5000000, point = point + 50000
where userid = 'eomjh';
...
----------------------------------------------------------------------------------------------
--- mdh2057 이라는 사용자가 제품번호 65 제품을 좋아한다에 투표를 한다.
--- 먼저 mdh2057 이라는 사용자가 제품번호 65 제품을 싫어한다에 투표를 했을 수도 있다.
--- 그러므로 먼저 tbl_product_dislike 테이블에서 mdh2057 이라는 사용자가 제품번호 65 제품이 insert 되어진 것을 delete 해야 한다.
...
JOIN tbl_order O
on D.fk_odrcode = O.odrCode  -- odrcode 는 전표
where D.fk_pnum = 65 and O.fk_userid = 'mdh2057'
...
where fk_userid = 'leess' and fk_pnum = 65;

commit;

-- ***** 특정제품에 대한 좋아요, 싫어요 투표 결과 (select) **** --
select ( select count(*)
         from tbl_product_like 
         where fk_pnum = 65 ) AS "좋아요 개수"
     , ( select count(*)
         from tbl_product_dislike
         where fk_pnum = 65 ) AS "싫어요 개수"
from dual;
...
select *
from tbl_orderdetail
where fk_odrcode || '/' || fk_pnum in('s20231627-13/4');

update tbl_orderdetail set deliverstatus = 1 
where fk_odrcode || '/' || fk_pnum in('s20210507-1/57','s20210507-1/6','s20210507-2/3');
-- 3개 행 이(가) 업데이트되었습니다.
commit;
