	   ...
	   if(checked_cnt < 1) {
		   ...
		   if( sum_totalPrice > current_coin) {
			   ...
			   if( confirm("총주문액 : "+ sum_totalPrice.toLocaleString('en') + "원 결제하시겠습니까?")) {
					   ...
					   $.ajax({
						   uri:"<%= request.getContextPath() %>/shop/orderAdd.up",
						   type:"post",
               ...
               <c:forEach var="cartvo" items="${requestScope.cartList}" varStatus="status"> 
                   <tr>
                        ...
                        <td align="right"> <%-- 주문총액/총포인트 --%> 
                            ...
                            <input type="text" class="totalPrice" value="${cartvo.prod.totalPrice}" />
                            <input type="text" class="totalPoint" value="${cartvo.prod.totalPoint}" />
                        </td>
