...
<jsp:include page="header.jsp" />    <%--  header_origin 을 가져온다, --%>
	...
	--%>
				...
				<c:forEach items="${requestScope.imgList}" varStatus="status">   			<%-- 4번이 돌아간다. --%>
		...
		--%>	    
			   ...
			   <c:forEach var="imgvo" items="${requestScope.imgList}" varStatus="status">     <%-- imgList 에 오라클에 저장되어있는 것들이 존재한다. var 는 파일 한개를 잡기 위해 --%>
...
<jsp:include page="footer.jsp" />    <%--  footer_origin 을 가져온다, --%>