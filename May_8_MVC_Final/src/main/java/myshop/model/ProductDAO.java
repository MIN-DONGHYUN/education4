...
public class ProductDAO implements InterProductDAO {
	...
	public boolean isOrder(Map<String, String> paraMap) throws SQLException {
		...
		try {
			...
			bool = rs.next();    // select 되어진것이 있냐 없냐 
								 // 있으면 true 없으면 false 리턴 한다.
		} finally {
			close();
		}
	...
	public int likeAdd(Map<String, String> paraMap) throws SQLException {
		...
		try {
			...
			conn.setAutoCommit(false);   // 수동 커밋으로 변경 함 
			// 이유는 좋아요 하기 전에 싫어요가 있다면 지워야 하므로 
			...
			n = pstmt.executeUpdate();
			//System.out.println("pdao 에서 n 값 : " +  n);

			
			if(n == 1) {
				conn.commit();   // 수동 커밋 하기 
			}
			
		} catch(SQLIntegrityConstraintViolationException e) {  // 제약조건에 위배되었다면  
			
			conn.rollback();  // 롤백하기 
			
		}finally {
			conn.setAutoCommit(true);   // 자동커밋으로 변경함
			close();
		}
	...
	@Override
	public Map<String, Integer> getLikeDislikeCount(String fk_pnum) throws SQLException {
	...
	} // end of public Map<String, String> getLikeDislikeCount(String fk_pnum) throws SQLException

	
	// Ajax 를 이용한 특정 제품의 구매후기를 작성하기(insert)
	@Override
	public int addReview(PurchaseReviewsVO reviewvo) throws SQLException {
		...
		try {
			...
			pstmt = conn.prepareStatement(sql);
			pstmt.setString(1, reviewvo.getFk_userid());
			pstmt.setInt(2, reviewvo.getFk_pnum());
			pstmt.setString(3, reviewvo.getContents());
	...
	} // end of public int addReview(PurchaseReviewsVO reviewvo) throws SQLException
	...
	} // end if public int reviewDel(String review_seq) throws SQLException
	...
	} // end of public List<HashMap<String, String>> getOrderList(String userid, int currentShowPageNo, int sizePerPage) throws SQLException
	...
	} // end of public List<HashMap<String, String>> selectStoreMap() throws SQLException
