	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
	   ...
	   //   HashMap 에 받는사람번호, 보내는사람번호, 문자내용 등 을 저장한뒤 Coolsms 클래스의 send를 이용해 보냅니다.
	     ...
	     //String api_key = "발급받은 본인의 API Key";  // 발급받은 본인 API Key
	      String api_key = "NCSZQHEFBZZYVDRL";     // 민동현꺼임  (api_key 는 보안이므로 없애고 블로그 올림 )
	     
	     //String api_secret = "발급받은 본인의 API Secret";  // 발급받은 본인 API Secret
	      String api_secret = "HDHHVNFTBZA6X4YFFZOQC1OWUBHAEDK1";  // 민동현꺼임 (api_secret 는 보안이므로 없애고 블로그 올림 )
	      ...
	      paraMap.put("from", "01051992057"); // 발신번호  (발신번호 는 보안이므로 없애고 블로그 올림 )
	   ...
	   //   paraMap.put("datetime", "20210422161000");   // 예약전송시 날짜 설정 반드시 YYYYMMDDHHMISS 형식대로 해야함.
	      ...
	      String datetime = request.getParameter("datetime");    // datatime 은 MemberOneDetail.jsp 에서 가져옴 
	      if(datetime != null) {
