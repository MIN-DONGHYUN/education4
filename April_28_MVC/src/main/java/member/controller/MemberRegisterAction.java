	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		// #### 회원가입이 성공되어지면 자동으로 로그인 되도록 하겠다. #### //
			...
			try {
				...
				if(n==1) {   // 값을 받아왔다면 즉 회원가입이 성공하였다면 
					...
					super.setRedirect(false);   // post 방식으로 우리가 해왔었다.
					super.setViewPage("/WEB-INF/login/registerAfterAutoLogin.jsp");		// 이동해서 보여줘라 
				}
				
			} catch(SQLException e) {
				
				e.printStackTrace();
				String message = "SQL 구문 오류 발생";
				String loc = "javascript:history.back()";    						 			 // 자바 스크립트를 이용한 이전페이지로 이동한다.
